...
/**
 * Invokes an Edge Function with the caller's session JWT. Privileged work (Auth admin
 * API, service-role key) stays server-side — the browser only ever holds the anon key.
 */
...
/**
 * Multipart variant for functions that take a file (`upload-document`). supabase-js sets
 * the multipart boundary itself, so the FormData is passed through untouched.
 */
